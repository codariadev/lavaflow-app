...
export default function Register() {
  ...
  const handleGoogleRegister = async () => {
    ...
    try {
      ...
      await handleRedirectByRole(uid);
    } catch (err: unknown) {
      if (err instanceof Error) {
        setError(err.message || "Erro ao registrar com o Google.");
      } else {
      ...
      <div className="relative z-0 flex flex-col min-w-0 wrap-break-words w-full min-h-screen justify-center items-center sm:w-auto sm:min-h-0 sm:justify-start sm:items-stretch bg-white border-0 shadow-none sm:shadow-soft-xl rounded-none sm:rounded-2xl bg-clip-border">
        <div className="w-full flex flex-col justify-center items-center">
          ...
          <div className="p-6 mb-0 text-center bg-white border-b-0 rounded-t-2xl text-slate-700 text-2xl sm:text-sm">
            <h5 className="font-bold">Registre-se no LavaFlow <p className="text-[12px] text-right">powered by <a className="underline" href="https://codariadev.vercel.app/">CodariaDev</a></p></h5>
          </div>

          {error && (
            <div className="p-3 mb-2 text-xs text-red-600 bg-red-50 border border-red-200 rounded-lg max-w-xs text-center">
              {error}
            </div>
          )}
          ...
          <div className="flex-auto p-6 flex justify-center w-full">
            <form onSubmit={handleRegister} className="w-[calc(100%+24px)] -mx-3 sm:w-full sm:mx-0" role="form">
              <div className="mb-4">
                <input
                  aria-label="Name"
                  placeholder="Nome"
              ...
              <div className="min-h-6 pl-7 mb-0.5 block">
                ...
                >
                  Eu aceito os{" "}
                  <a className="font-bold text-slate-700 underline cursor-pointer">Termos e condições</a>
                </label>
              </div>
